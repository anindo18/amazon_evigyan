...
<html>
    ...
    <body>
        <form name="form" action="take_chem_quiz_next.php" method="post">
            <?php
             session_start();
            if(isset($_SESSION['views_chem_quiz']))
            $_SESSION['views_chem_quiz']=$_SESSION['views_chem_quiz']+1;
            else
            $_SESSION['views_chem_quiz']=1;
            echo "Number of questions attempted is :". $_SESSION['views_chem_quiz']."<br>";
            if( $_SESSION['views_chem_quiz']>10)
            {
                header("Location: close_chem_quiz.php");
            }
            echo "<br>";
            echo "<hr>";
            echo "<p style='left: 1000px; font-size: 17px;'><b><i>".$_COOKIE["user"]." is logged in</i></b></p>" ;
        echo "<hr>";
            ?>
            <div class="box">
            <?php
        ...
        $result1 = mysqli_query($con,"select * FROM quiz_chemisrty");
...
echo "Total rows: " . $num_rows;
        ...
        $count1= $random1;
  $result = mysqli_query($con,"SELECT id, question, option1, option2, option3,option4,answer FROM quiz_chemisrty where id=$random1");
